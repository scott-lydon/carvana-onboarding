...
1. **AI Assessment Module with LLM-powered chatbot capabilities.** Conversational onboarding shell that drives the entire sell-side flow as a chat instead of a multi-screen form. Uses Anthropic Messages API with **Claude Haiku 4.5** (`claude-haiku-4-5`, overridable per environment via `ANTHROPIC_MODEL`) as the orchestrator and tool-use to invoke VendorCascade for plate/VIN lookups, OcrService for photo capture and document extraction, ConditionExtractor for AI-vision condition assessment from photos, Scheduler for pickup booking, OfferEngine for the deterministic instant-offer formula, PayoffCalculator for negative-equity math, ContractAcknowledger for single-signature consent on POA + bill of sale + odometer disclosure, and SupportContent for empathy interstitials. Haiku 4.5 was chosen over Sonnet 4.5 explicitly so first-token latency is 300-600ms and the UI can drop the four-phase progress bar in favor of a minimal typing-dot indicator inside the empty assistant bubble.
...
**US-V2-8 — Photo-driven condition intake (Slice F).** As a seller, I want to upload a small handful of photos (four exterior corners + odometer + interior + VIN plate + any damage closeups) and have AI extract my mileage, tag visible damage, suggest a condition tier, and only ask me follow-up questions for things photos cannot show (warning lights, smoke, mechanical issues, accident history, modifications).
Given the vehicle has been resolved by `lookup_plate` or `lookup_vin`. When the chatbot calls `start_condition_intake`. Then the multi-photo uploader opens (`ConditionIntake.tsx`) with 9 angle slots, the seller fills at least 3 (max 12), and on submit `/api/condition/extract` runs Claude vision over all images in one call. The response includes `extractedMileage` (integer), `odometerConfidence` (0-1), `visibleDamage[]` (panel + severity + note), `suggestedTier`, and `followupQuestions[]` (max 4). The seller answers any follow-ups inline in the chat. The full result lands in the SellWorkspace's "Condition" slice.

**US-V2-9 — Loan status & payoff capture (Slice F).** As a seller with an outstanding loan, I want the chatbot to ask me explicitly whether I have a lien, then collect my lender name and 10-day payoff amount so my instant offer accounts for negative or positive equity.
Given the condition assessment is complete. When the chatbot calls `record_loan_status({hasLien:true})`. Then the `PayoffForm` panel opens with two fields (lender name, 10-day payoff amount). On submit the values land in the SellWorkspace's "Title & Loan" slice and the chatbot uses them as `payoffAmount` on the next `generate_offer` call. When `hasLien:false`, the tool returns immediately and the workspace shows "No lien — title is clean".

**US-V2-10 — Transparent, deterministic instant offer (Slice F).** As a seller, I want my offer to be a real number derived from a published formula I can audit live, not a fabricated dollar amount.
Given year, make, model, mileage, condition, and (optionally) payoff have been collected. When the chatbot calls `generate_offer({year, make, model, mileage, condition, payoffAmount?})`. Then `server/offer/OfferEngine.ts` computes the offer from the explicit formula `(makeTier × yearDepreciation + mileageAdj) × conditionMultiplier - payoff`, rounded to the nearest $50. The response is an `OfferResult` with a line-itemed breakdown (`lines[]`) showing every factor's dollar contribution and a one-sentence explanation. The UI renders this as an `OfferCard` next to the chatbot's message. Offer is valid 7 days or 1,000 miles, whichever first (mirrors Carvana's actual policy). Negative equity surfaces as a red callout naming the exact cashier's-check amount.

**US-V2-11 — Payment method selection (Slice F).** As a seller with an accepted offer, I want to choose how I get paid: ACH (direct deposit 1-2 business days), check at pickup, or trade-in credit toward a Carvana purchase.
Given pickup is booked. When the chatbot calls `select_payment_method`. Then the `PaymentMethod` panel opens with three radio cards. On selection, the choice lands in the SellWorkspace's "Payment" slice and surfaces as a chat message the chatbot can acknowledge.

**US-V2-12 — One-click contract acknowledgement (Slice F).** As a seller closing the deal, I want to acknowledge the Limited Power of Attorney, Bill of Sale, and Federal Odometer Disclosure on a single page with one checkbox, instead of three separate signing flows.
Given payment method is selected. When the chatbot calls `acknowledge_contract`. Then the `ContractConsent` panel renders the three disclosures in plain English, the seller checks one box covering all three, and the acknowledgement (`{acknowledgedAt, items: ["poa","bos","odometer"]}`) lands in the SellWorkspace's "Contract" slice.

**US-V2-13 — Live SellWorkspace aggregator (Slice F).** As a seller mid-flow, I want a glanceable summary panel that shows what data has been captured so far and what's left, so I never lose my place in the conversation.
Given any stage has been completed. When the seller looks at the chat. Then `SellWorkspace.tsx` renders below the chat composer with seven sections (Vehicle, Condition, Title & Loan, Instant Offer, Pickup, Payment, Contract) each showing a gray "Not started" or green "Complete" pill plus a one-line preview of the captured value. The workspace is hidden on the empty landing chat (no clutter) and appears the moment any stage has data.
...
| <3 s response time under load (with stretch target: 1 s p50 TTFT, 2 s p95 TTFT on Haiku 4.5 — no progress bar required) | k6 load test against the deployed Render instance (`scripts/perf/load.k6.js`), p95 reported in `docs/perf-report.md`. Render service pinned to paid tier OR pre-warmed before the demo to dodge cold-start penalty. The TTFT (time-to-first-token) is the meaningful number now — Haiku 4.5 delivers a token before a human can read the user's own message, so the chat needs no multi-stage progress affordance. | `scripts/perf/load.k6.js`, `docs/perf-report.md` |
