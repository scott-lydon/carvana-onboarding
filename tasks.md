...
- [ ] **A.8** Invoke qa-adversary against slice A in fresh context. IN PROGRESS: claude-code-bridge delegation spawned (PIDs 77960/77961) but MCP timed out at 600s while the agent kept running; report will land at `docs/qa-reports/slice-A.md`. Inline adversary self-check PASS in the meantime (CAT-1, CAT-3, CAT-8, CAT-11, CAT-12, CAT-13, CAT-17 PASS; rest N/A for slice A).
- [ ] **A.9** Submit-gate at end of slice-A response. BLOCKED on `ANTHROPIC_API_KEY` in `.env.local` (and in Render env for deployed); without the key /api/chat returns 503 and the primary user story cannot walk live. Re-runs to PASS automatically when user pastes the key.
