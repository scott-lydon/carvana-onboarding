...
## Slice EVAL — chat-accuracy eval harness (2026-05-25, COMPLETE)

> Surfaced by a user audit ("do they have evals?") that landed on the gap between the existing unit / property / integration / adversary layers and the question "does the live model still route an empathy question through the support-content tool?" The integration-test layer answers code-path questions; the eval layer answers behaviour questions about the model + prompt + tool wiring as a whole. Distinct concerns warrant distinct test layers.

- [x] **EVAL.1** Build the harness types and scorer at `tests/eval/types.ts` + `tests/eval/harness.ts`. Predicates available per fixture: `mustCallTools`, `mustCallToolsInOrder`, `mustNotCallTools`, `toolResultKindIs`, `toolResultMustHaveKeys`, `proseMustContainAny`, `proseMustNotContain`, `proseMustMatch`, `minToolUseCount` / `maxToolUseCount`. All hard predicates; no LLM-as-judge. Auto-skips when `ANTHROPIC_API_KEY` is unset; per-fixture skip when listed `requiredEnv` (such as `CARSXE_API_KEY`) is missing. Done.
- [x] **EVAL.2** Wire 11 JSON fixtures across 6 rubric pillars at `tests/eval/fixtures/`: tool_routing (3), pii_gate (2), empathy_routing (2), facts_routing (2), flow_ordering (1), format_error_recovery (1). Each fixture carries an `id`, `category`, `description`, `turns[]`, `assertions`, and optional `requiredEnv`. The harness validates schema at load — a malformed fixture fails LOUDLY rather than silently skipping. Done.
- [x] **EVAL.3** Vitest entry point at `tests/eval/chat-accuracy.eval.ts`. The eval file uses the `.eval.ts` suffix and lives under a dedicated `tests/eval/vitest.eval.config.ts` so the default `npm test` ignores it (no live LLM calls on every commit). New npm script `test:eval` runs the eval against a single Express server hosting the production chat handler + real `VendorCascade`. Failures dump the full assistant prose AND the structured tool_calls observed, so a grader can diagnose a regression without re-running the fixture. Done.
- [x] **EVAL.4** README at `tests/eval/README.md` documents how to run, how to add a fixture, and why every predicate is deterministic (no judge model). Done.
- [x] **EVAL.5** First live run: `npm run test:eval` against Haiku 4.5 + CarsXE passes 11 of 11 fixtures in 33s. Per-pillar tally is printed in the test log: tool_routing 3/3, pii_gate 2/2, empathy_routing 2/2, facts_routing 2/2, flow_ordering 1/1, format_error_recovery 1/1. Cost per full run ≈ $0.01 at Haiku rates. Done.
- [x] **EVAL.6** Default suite confirms no regression: `npm test` runs 217 of 217 existing tests, with `tests/eval/**` excluded by both the include filter (`*.test.ts` only) and an explicit `exclude` entry in `vite.config.ts`. Done.

## v2 Slice F-Flow — full sell-side post-VIN flow (COMPLETE, 2026-05-24)

> Added in response to user feedback: "we haven't filled the requirements — we don't cover all the forms that follow the VIN/license entry; please revolutionize the whole sell-your-car flow." Carvana's real sell flow has 14 stages (researched against carvana.com help center + Reddit walkthroughs + 2025 CT AG settlement reporting); we previously covered 2 (lookup + scheduling). Slice F-Flow adds the missing 7 seller-facing stages: condition intake (photo-driven), loan/payoff capture, instant offer (deterministic formula, transparent breakdown), payment method, contract acknowledgement, and the live SellWorkspace aggregator. Stage 12 (in-person inspection) and stage 14 (mailed title transfer) stay out of scope by design — they happen IRL, not in the web flow.

- [x] **F.1** Swap chat model from `claude-sonnet-4-5` to `claude-haiku-4-5` (env-overridable via `ANTHROPIC_MODEL`). Done: `server/routes/chat.ts:45-62`; sub-second TTFT eliminates the four-phase ProgressBar from the chat transcript (US-V2-4 was hiding a 1.5-3s gap with progress chrome).
- [x] **F.2** Remove the in-transcript `<ProgressBar>` from `ChatbotShell.tsx`; replace with a 3-dot typing indicator inside the empty assistant bubble (keyframes in the shell's `<style>` block, respects `prefers-reduced-motion`). Done: `src/components/ChatbotShell.tsx` — `chatbot-typing-dots` class + `chatbot-typing-pulse` keyframes.
- [x] **F.3** Build the OfferEngine — pure deterministic formula `(makeTier × yearDepreciation + mileageAdj) × conditionMultiplier - payoff`, rounded to nearest $50, valid 7 days / 1,000 mi (mirrors Carvana). Done: `server/offer/OfferEngine.ts`, formula version `2026-05-24.v1`. Every line in the returned `OfferResult.lines[]` carries the dollar contribution + a one-sentence explanation the UI renders verbatim.
- [x] **F.4** Build `/api/offer/generate` (pure compute, no key needed; per-field validation, errors name the field + reason + fix). Done: `server/routes/offer.ts`.
- [x] **F.5** Build `/api/condition/extract` — multi-image Claude vision (3-12 photos in one call). Returns extracted mileage + odometerConfidence + visibleDamage[] + suggestedTier + 1-4 followupQuestions. HEIC → sharp pipeline reused from `/api/ocr/recognize`. Done: `server/routes/condition.ts`.
- [x] **F.6** Wire 5 new chat tools (`start_condition_intake`, `record_loan_status`, `generate_offer`, `select_payment_method`, `acknowledge_contract`) with REAL dispatchers (no sentinels). `generate_offer` calls `OfferEngine.generateOffer` directly; the four panel-backed tools return `user_action_required` with an action ID the client routes to the right panel. Done: `server/chat/tools.ts`; CAT-17 tool-count test bumped from 5 to 10.
- [x] **F.7** Rewrite system prompt for the 9-stage flow (greeting → lookup → condition → follow-ups → loan → offer → pickup → payment → contract → wrap). Done: `server/chat/system-prompt.ts` (re-written; the PII-out-of-text rule and the do-not-paraphrase-support-content rule are preserved verbatim).
- [x] **F.8** Build 6 React components: `ConditionIntake.tsx` (9-slot uploader, HEIC-safe base64, dots indicator), `OfferCard.tsx` (pure render of `OfferResult` with red/green callouts for negative equity / net to seller), `PayoffForm.tsx` (lender + 10-day payoff), `PaymentMethod.tsx` (ACH / check / trade credit), `ContractConsent.tsx` (single-checkbox for POA + bill of sale + odometer disclosure), `SellWorkspace.tsx` (7-section aggregator). Done: all six under `src/components/`; each follows the `useScheduler` / `useOcrCapture` hook+panel pattern.
- [x] **F.9** Wire the 6 components into `ChatbotShell.tsx`: hooks, refs for imperative `open()`, SSE event handler watches `user_action_required.action` and opens the matching panel; SSE handler captures `generate_offer` result into `workspaceState.offer`; lookup/condition/payoff/pickup/payment/contract callbacks all update `workspaceState` AND inject the structured chat message that the chatbot picks up on the next turn. `SellWorkspace` renders only when at least one slice has data. Done.
- [x] **F.10** Mount `/api/offer/generate` (no key needed) and `/api/condition/extract` (503 fallback when `ANTHROPIC_API_KEY` missing, parallel to the existing chat / OCR fallbacks). Done: `server/index.ts`.
- [x] **F.11** Update the CAT-17 tool-count adversary test to expect the 10-tool world AND name every tool the dispatcher must handle. Done: `tests/adversary/CAT-17-sliceA-malformed-tool-input-with-cascade.test.ts`.
- [x] **F.12** Typecheck (client + server tsconfigs) + lint + 69 vitest pass on the host Mac. Done.

### Slice F-Flow followups (queued for next slice)

- [ ] **F.13** Add a vitest unit suite over `OfferEngine.generateOffer` — deterministic outputs for canonical inputs (2021 F-150, 47k mi, Good, no lien = a specific dollar number), monotonicity properties (more miles = lower offer, better condition = higher offer), rounding-to-50 invariant, payoff subtraction edge cases (payoff = subtotal, payoff > subtotal). Property tests with fast-check.
- [ ] **F.14** Add `/api/condition/extract` integration test using a multi-image fixture (the four-corner photos from `test-plates/` if available; otherwise commit small JPEG stubs). Assert HEIC fallback. Assert shape coercion against a known vision response captured by hand.
- [ ] **F.15** Playwright end-to-end happy path covering ALL 9 stages: chat → plate → condition photos → follow-up answers → no-lien → offer → pickup → payment → contract → wrap. Stopwatch ≤ 5 min.
- [ ] **F.16** Architecture website update — add three new boxes to the Mermaid topology (OfferEngine, ConditionExtractor, SellWorkspace), add three new rows to the decision table (deterministic-vs-fabricated offer; single-call vs per-image vision; right-rail aggregator vs in-line tool cards), add an "AI-revolution targets" section quoting the 6 friction points researched from carvana.com Help / Reddit / CT AG settlement.
- [ ] **F.17** Document the formula in `docs/offer-formula.md` (with the year-depreciation curve sourced from Edmunds + the make-tier base table) so reviewers can audit it without reading code.
- [ ] **F.18** Slice F-Flow qa-adversary in fresh context.
